...
/*   Updated: 2024/10/07 15:11:57 by mcuenca-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	concat = "\0";
	if (fd < 0 || fd >= MAX_FD || BUFFER_SIZE <= 0 || BUFFER_SIZE > 2147483646)
		return (NULL);
	if (buffer[fd])
	{
		concat = ft_strjoin(concat, buffer[fd]);
		free(buffer[fd]);
	}
	buffer[fd] = ft_calloc(BUFFER_SIZE + 1, sizeof(char));
	if (!buffer[fd] || !concat)
		return (NULL);
	tmp = fill_phrase(fd, buffer[fd], concat);
	if (!tmp)
		return (NULL);
	buffer[fd] = keep_and_remain(tmp, ft_strlen(tmp));
